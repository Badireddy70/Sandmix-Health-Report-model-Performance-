...
datapath = filepath + "\data\weightedOptimization"
...
input_data_block_total = pd.read_excel(datapath+"\\input_disa70.xlsx")
output_data_block_total = pd.read_excel(datapath+"\\output_disa70.xlsx")
...
#i have commented here
input_data_block=input_data_block_total.ix[input_data_block_total.index<data_lentrain]
########
########
...
l=0
input_data_blocktest=input_data_block_total.ix[input_data_block_total.index>l]
input_data_blocktest.index=range(len(input_data_blocktest))
output_data_blocktest=output_data_block_total.ix[output_data_block_total.index>l]
output_data_blocktest.index=range(len(output_data_blocktest))
#    
Date_shift_test=Date_shift.ix[input_data_block_total.index>0]
...
modeling_date=Date_shift.ix[input_data_block_total.index<data_lentrain]
#######
#    #
#    #####################################################################
#    ######################################################################
...
def estimateModelParameters(input_data_block,output_data_block,retainedInputEigenVectors):   
    ...
    datainput,inputMean = meanCenterData(scaledInputData)
    dataoutput,outputMean = meanCenterData(scaledOutputData)
    inputScores = projection2ReducedSpace(datainput[input_columnlist].as_matrix())
    output =dataoutput[outputFieldsList].as_matrix()
...
#coeff.to_csv("C:\Users\ADMIN\Desktop\Office work\Nelcast.Gudur\gudur\Modelcoefficientsdisa70new.csv")

   
def predict(datainput):
    data = copy.deepcopy(datainput[inputFieldsList])
    data = data.reindex_axis(inputFieldsList,axis=1)
    data = NormalizeColumns(input_columnlist,data,dataInputMinMaxDataFrame)
    data = data -inputMean[input_columnlist]
    prediction = np.dot(data.as_matrix(),modelCoefficientsInOriginalSpace)
...
##        #==========================================================modified on 20.09.2016
    optX1 = optX.reindex_axis(outputFieldsList,axis = 0)
    #        #==========================================================
    prevX1 = prevX.reindex_axis(delayedoutputfieldlist,axis = 0)#.as_matrix()
    #        
    ...
    #        
    #                
    optX1 = optX1.as_matrix()
    prevX1 = prevX1.as_matrix()
    #        
    ...
    #        
    h1 = np.zeros((nAdditives)) -inputMean[inputFieldsList2].reindex_axis(inputFieldsList2, axis = 0)
    h2 = np.ones((nAdditives)) - inputMean[inputFieldsList2].reindex_axis(inputFieldsList2, axis = 0)
...
#    
    ...
    A = delta[inputFieldsList2].reindex_axis(inputFieldsList2,axis = 0).as_matrix()
...
for i in range(len(input_data_blocktest)):
    ...
    p_opt=[8.99,40.55,1562.58,4.89,3.14,137.92,78.55,21.82]
...
##########################################################################################    
Group_info=list(['ALL'])
targets=pd.DataFrame([8.99,40.55,1562.58,4.89,3.14,137.92,78.55,21.82],columns=['ALL'])
targets.index=outputFieldsList
Handals={'Bentonite':'None','Fresh Silica Sand':'None','LCA':'LOI (%)','Return Sand':'None','Water':'None'}
pred_coeff=pd.DataFrame([99999,99999,0.5,99999,99999],columns=['ALL'])
...
for i in range(len(Group_info)): 
    ...
    for j in range(len(Group_input)):         
            if (Handals[Group_input[j]])=='None':
                x[:,j]= predicted_additive[predicted_additive_column[j]].as_matrix()
                ...
                modify_string=Group_input[i]+str('pred')
                x[:,j]=(pd.DataFrame([(predicted_additive[predicted_additive_column[j]]+(dev/pred_coeff[Group_info[i]][Group_input[j]]))]) ).as_matrix()                    
...
for key in Group_additives:
      ...
      data_for_pred=data_for_pred.reindex_axis(inputFieldsList, axis=1)
...
def sand_pred(df):
    ...
    pred = np.dot(df_new.as_matrix(),beta.T)
...
actual_predicted=pd.concat([Date_shift_test,input_data_blocktest[['Active Clay (%)','GCS (kg/cm2)','LOI (%)','Wet Tensile Strength (gm/cm2)']],input_data_blocktest[acual_additive_column],predicted_additive_without_adjustment[predicted_additive_column],adjust_additive_pred],axis=1)
